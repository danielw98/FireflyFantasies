...
public:
    ...
    void UpdateBoardAfterSelection(void)
    {
        ...
        if (clearSelection)
        {
            ...
            // firefly[i][j]'s that should be set to null - could be a set of Firefly*
            std::set<std::pair<int, int>> indicesToNull;
            std::list<std::pair<std::pair<int, int>, Firefly*>> indicesToFill;
            std::set<std::pair<int, int>> indicesThatFell;
            for (int i = 0; i < sizeX; i++)
            {
                int saved_j = 0;
                for (int j = 0; j < sizeY; j++)
                {
                    ...
                    if(numNulls > 0)
                    {
                        if(board->GetFirefly(i, j) != nullptr)
                            indicesToNull.insert(std::make_pair(i, j));
                        indicesToFill.push_back(std::make_pair(std::make_pair(i, j+numNulls), firefly));
                        saved_j++;
                    }
                }
                for(int j1 = 0; j1 < saved_j; j1++)
                    indicesThatFell.insert(std::make_pair(i, j1));
            }
            ...
            for (int i = 0; i < sizeX; i++)
            {
                for (int j = 0; j < sizeY; j++)
                {
                    if (board->GetFirefly(i, j) == nullptr)
                    {
                        ...
                        Firefly *newFirefly = new Firefly(i, j, highlighted);
