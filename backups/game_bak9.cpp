...
public:
    ...
    void UpdateBoardAfterSelection(void)
    {
        ...
        if (clearSelection)
        {
            ...
            // the first time the level finishes, we show the levelover firefly
            if(board->IsLevelOver())
            {
                board->SetLevelOverFirefly(true);
                board->SetFinalFirefly(new Firefly(TextureManager::GetInstance()->GetFinalFirefly(), sizeX));
            }
            // when showing the levelover firefly, all cells are not highlighted
            if(board->ShowLevelOverFirefly())
            {
                for (int i = 0; i < sizeX; i++)
                {
                    for (int j = 0; j < sizeY; j++)
                    {
                        board->GetFirefly(i, j)->SetHighlighted(false);
                    }
                }
                showingLevelOverFirefly = true;
            }


            if(board->IsLevelOver() && showingLevelOverFirefly)
            {
                if(count++ == 3)
                    showingLevelOverFirefly = false;
            }
            // go to next level when the level is over and the level over firefly left the screen
            if(board->IsLevelOver() && !showingLevelOverFirefly)
            {
                delete board;
                count = 0;
